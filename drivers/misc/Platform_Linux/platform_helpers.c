...
struct fusb30x_chip* chip = fusb30x_GetChip();
    ...
    if (!chip)
    {
    	...
    	power_supply_set_usb_otg(chip->usb_psy, set ? 1 : 0);
   //   printk("zou_hao_set_vbus****************\n");
}
...
FSC_BOOL fusb_reset(void)
{
    ...
    if (!fusb_I2C_WriteData((FSC_U8)0x0C, 1, &val))
    {
        pr_err("FUSB  %s - Error: Could not communicate with device over I2C!\n", __func__);
        return FALSE;
    }
...
FSC_S32 fusb_EnableInterrupts(void)
{
   ...
   enable_irq_wake(chip->gpio_IntN_irq);
	...
	//zouhao
	#if 0
	ret = devm_request_irq(&chip->client->dev,chip->client->irq,
				_fusb_isr_intn,
				IRQF_TRIGGER_FALLING,
				"fusb302_int_irq", chip);
	if (ret) {
...
down(&chip->suspend_lock);
   //pr_err("zou_hao_isr_intn******\n");
  // wake_lock_timeout(&chip->wlock,msecs_to_jiffies(FUSB302_WAKE_LOCK_TIMEOUT));
//pm_stay_awake(&chip->client->dev);
    ...
    core_state_machine();  
up(&chip->suspend_lock);// Run the state machine
